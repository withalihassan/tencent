...
$result = $mysqli->query(
    "SELECT id, account_id, secret_id, secret_key, created_at
     FROM api_keys
     WHERE by_user = '" . $mysqli->real_escape_string($current_user) . "'
            ...
            <table id="keysTable" class="table table-striped" style="width:100%">
                ...
                <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td><a href="./sub_users.php?keyid=<?= htmlspecialchars($row['id']) ?>&userid=<?= $current_user ?>&acid=<?= htmlspecialchars($row['account_id']) ?>" target="_blank" class="btn btn-sm btn-info">Open</a></td>
                    </tr>
                <?php endwhile; ?>
                </tbody>
            </table>
